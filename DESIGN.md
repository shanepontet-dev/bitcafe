...
### Named Rules
**The Two-Ink Rule.** Only red and teal ever carry color. If a third hue is needed for a new component, the system has been misapplied — reach for weight, size, or the existing inks first. The one carved-out exception is the Pixel Cup's full-color illustrated sprite (see Components below) — a standalone authored asset, not a precedent for coloring UI chrome.
...
A 9-frame hand-authored pixel-art sprite (`img/coffee/cup-1.png`…`cup-9.png`, 256×256, `image-rendering: pixelated`) used on `coffee.html`: a full-color illustrated coffee cup and saucer with steam looping continuously across the 9 frames, swapped on a plain `<img>` via `setTimeout`, not canvas-drawn. This is the system's one deliberate exception to the Two-Ink Rule below — the art is full color (cream, browns, a teal trim band, soft blue-grey steam), because it's an authored illustration standing in for itself, not a new UI color introduced for a new control. It carries no brew-progress state (no empty/full levels); progress is communicated entirely by the status line and brew bar instead, and the cup simply steams from the moment brewing starts through the finished receipt. `canvas` is still used, but only offscreen, to composite the current frame onto an opaque paper backing for the "save as .jpg" / "save as .gif" export pair — the live on-page cup itself is never a canvas.

### Button Wall (signature component)
A wrapped row of 88×31 badges on `links.html` (plus a same-footprint teaser of the newest ones on the homepage) — entirely admin-managed now: every badge is uploaded from `/admin`'s webring tab (`wall_buttons`, see README.md) rather than hand-coded into the page, and the wall is empty until fletcher adds one. Each is real button art fletcher made or has rights to, rendered plain at 88×31 with no inline animation or template — the "Don't re-host" rule below still applies: never another site's own logo or badge lifted off their page without asking. A "wander somewhere random" control sits beside the wall and opens a random pick from the same linked-site pool (the "sites we love" coupon strip's picks) in a new tab; treat it as part of this component, not a separate button style.
...
### Don't:
- **Don't** re-host or trace another site's actual logo, badge, or button art. The button wall's badges are art fletcher actually made or has rights to — real enough to credit the site by name and link there for real, never a copy of assets we don't have rights to.
- **Don't** reach for `canvas` by default. The pixel cup's export pair is the one exception, justified by a concrete need (compositing a frame to an opaque backing for image/GIF export) that CSS/SVG can't satisfy — not a precedent for redoing other components in canvas. The live pixel cup itself is a plain `<img>`, not canvas.
